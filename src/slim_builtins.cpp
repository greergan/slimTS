...
void slim::builtins::initialize(v8::Isolate* isolate, v8::Local<v8::ObjectTemplate>& globalObjectTemplate) {
	trace(Message("slim::builtins::initialize()","begins",__FILE__, __LINE__));
	//v8::HandleScope scope(isolate);
	trace(Message("slim::builtins::initialize()","after handle scope",__FILE__, __LINE__));
	slim::dummy_console::expose_plugin(isolate);
	trace(Message("slim::builtins::initialize()","after expose_plugin",__FILE__, __LINE__));
	slim::plugin::plugin slim_objects(isolate, "slim");
	trace(Message("slim::builtins::initialize()","after slim",__FILE__, __LINE__));
...
void slim::builtins::require(const v8::FunctionCallbackInfo<v8::Value>& args) {
	trace(Message("slim::builtins::require","begins",__FILE__, __LINE__));
	...
	debug(Message("slim::builtins::require",std::string("plugin name => " +  plugin_name_string).c_str() ,__FILE__, __LINE__));
	...
	if(!v8_property_value.IsEmpty() && v8_property_value->IsObject()) {
		...
		trace(Message("slim::builtins::require","found object",__FILE__, __LINE__));
	}
	trace(Message("slim::builtins::require","ends",__FILE__, __LINE__));
}