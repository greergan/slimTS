...
std::string slim::macros::apply(const std::shared_ptr<std::string> file_contents, const std::string& absolute_path_to_file) {
	if(!file_contents.get()->empty()) {
		std::stringstream file_contents_stream(*file_contents.get());
